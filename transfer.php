        ...
        if($nums > 0 && $nums > 0)
        {
            ...
            if($credit < $rows['balance'])
            {
                $okok = true;
                    if($okok)
                    {
                        $qrys = "UPDATE `bank`.`users` SET `balance` = `balance` - '$credit' WHERE `id` ='$sid' ";
                        $qryr = "UPDATE `bank`.`users` SET `balance` = `balance` + '$credit' WHERE `id` ='$rid' ";
